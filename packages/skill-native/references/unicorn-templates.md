...
## 模板 1: IDAPython 脚本（IDA headless 模式）
...
description:
  ...
  IDA_FUNC_ADDR=0x401000 IDA_OUTPUT=task-dir/result.json \
...
> **必须使用 `python` 运行**（需要 unicorn 包，安装在 venv 中）。
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(f"用法: python {sys.argv[0]} <二进制文件> <函数偏移(hex)> <函数大小>")
        sys.exit(1)
