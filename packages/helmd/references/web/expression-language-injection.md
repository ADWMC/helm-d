...
- EL/SpEL/OGNL RCE payload 与沙箱绕过 → 下文对应 REF 章节
...
> 融合自 yaklang/hack-skills `expression-language-injection`，与上文去重后仅保留独有内容。检测/SpEL/OGNL/Java EL 基础 RCE 见上文 §1-6；Cloud Gateway CVE-2022-22947 完整链见上文 §2；Confluence CVE-2021-26084 探测见上文 §3；沙箱绕过与 MVEL/盲注见 REF: el-exploitation。

## 引擎歧义判定矩阵

| `${7*7}` 响应 | `%{7*7}` 响应 | 引擎 |
|---|---|---|
| 49 | 字面量 `%{7*7}` | SpEL 或 Java EL |
| 字面量 `${7*7}` | 49 | OGNL (Struts2) |
| 49 | 49 | 两者都活 |

辅助探测：`${T(java.lang.Math).random()}` 返回随机浮点 = SpEL 实锤；`%{#context}` 吐对象 = OGNL 实锤。

## SpEL 出现位置（找注入面）

- `@Value("${...}")` 注解
- Spring Security 表达式（`@PreAuthorize`）
- Spring Cloud Gateway 路由 predicate / filter
- Thymeleaf `th:text="${...}"`（配合 `__${...}__` 预处理）
- Spring Data `@Query` 中的 SpEL

## OGNL 出现位置

- Apache Struts2（最大消费方）
- Confluence Server（特定请求路径）
- 任何直接调用 `ognl.Ognl.getValue() / setValue()` 的应用

## Struts2 关键 CVE 速查

| CVE | 向量 | Payload 位置 |
|---|---|---|
| S2-045 (CVE-2017-5638) | Content-Type 头 | `%{...}` 在 Content-Type |
| S2-046 (CVE-2017-5638) | Multipart 文件名 | 上传文件名中的 OGNL |
| S2-016 (CVE-2013-2251) | `redirect:` / `redirectAction:` 前缀 | URL 参数 |
| S2-048 (CVE-2017-9791) | Struts Showcase | ActionMessage 中的 OGNL |
| S2-057 (CVE-2018-11776) | Namespace OGNL | URL 路径 |
