...
详见 `playbooks/file-upload/00-index.md` 与本目录 `13-file-rce-chain.md`。
...
实战中可调用 jshookmcp 完成自动化。**默认 `search` profile 未预加载工具,调用前先用 `mcp__jshook__activate_tools <工具名>` 激活**(详见 `tools/mcp-jshook.md` §推荐 profile)。
