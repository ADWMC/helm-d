...
- 云服务商接管模式与 DNS 记录类型速查 → 下文对应 REF 章节
...
> 融合自 yaklang/hack-skills `subdomain-takeover`，与上文去重后仅保留独有内容。核心概念/检测流程/NS/MX/通配符/决策树见上文 §1-9；Azure/Shopify/Fastly/Zendesk/Fly/Surge 指纹见 REF: takeover-techniques。

## 自动化工具表

| 工具 | 用途 |
|---|---|
| `subjack` | CNAME 接管自动检测 |
| `nuclei -t takeovers/` | Nuclei 接管检测模板 |
| `can-i-take-over-xyz` (GitHub) | 各服务可接管性速查参考 |
| `dnsreaper` | 多供应商接管扫描 |
| `subzy` | 快速验证 |

## 供应商指纹补充（REF 表之外的供应商）

| 供应商 | CNAME 模式 | 指纹（HTTP 响应） | 可认领？ |
|---|---|---|---|
| Pantheon | `*.pantheonsite.io` | `404 Site Not Found`（Pantheon 品牌） | 是 |
| Tumblr | `*.tumblr.com`（自定义域 CNAME） | `There's nothing here` | 是 |
| WordPress.com | CNAME → `*.wordpress.com` | `Do you want to register` | 是（WP.com 认领域名） |
| Unbounce | `*.unbouncepages.com` | `The requested URL was not found` | 是 |
| Ghost | `*.ghost.io` | `404 Not Found`（Ghost 报错） | 是 |

## 防御与整改

| 措施 | 优先级 |
|---|---|
| 下线云资源时同步删除 DNS 记录 | Critical |
| 监控 CNAME 目标是否出现 NXDOMAIN | High |
| DNS 监控工具（SecurityTrails / DNSHistory） | High |
| 删 DNS 记录前先认领/占位资源名 | High |
| 审计 NS 委派 —— NS 域名必须自有且续费 | Critical |
| 避免把通配符 CNAME 指向第三方服务 | Medium |
| 启用 Certificate Transparency 监控 | Medium |

## AI 模型经常漏掉的要点

1. **有 CNAME ≠ 可接管**：CNAME 指向 S3 返回 403（桶存在且私有）不可接管，只有 `NoSuchBucket`（404）才算。
2. **S3 区域有讲究**：桶名全局唯一，但 website 端点按区域 —— 从 CNAME 里推断区域再建桶。
3. **GitHub Pages 已加域名验证**：组织验证过的域名别人认领不了 —— 先确认目标是否启用了验证。
4. **个别供应商（如 CloudFront）需要特定分发配置**，不是单纯认领域名就行。
5. **二阶接管**：`sub.target.com CNAME → other.target.com CNAME → dead-service.com` —— 链条必须走到底。
6. **SPF 子域接管**：SPF 里 `include:sub.target.com` 且你接管了 `sub.target.com` → 改它的 SPF TXT 授权你的邮件服务器 → 以 `target.com` 身份发伪造邮件。
