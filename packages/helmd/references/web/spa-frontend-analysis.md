...
| 当...时 | 使用本节 |
|---------|---------|
| 识别到纯前端 SPA 应用 | §1 纯前端 SPA 架构识别 |
| 分析 SvelteKit 路由 | §2 SvelteKit 路由分析 |
| 遇到 Notebook 导入功能 | §3 Notebook 导入攻击面 |
| 分析 Bot localStorage 变体 | §4 Bot localStorage 利用 |
| 需要异步 flag 时间差利用 | §5 异步 flag 时间差 |
...
> 完整的攻击编排模板（含 iframe 逃逸、postMessage 通信、错误处理），见 `attack-orchestration.md` §3.3。
...
> 完整的 Bot 行为模式分析、利用条件、攻击编排模板，见 `attack-orchestration.md` §3 "Bot 时间差利用"。
