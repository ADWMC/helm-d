...
> **SRC / 众测语境** → 用 `read_reference(path: "src-hunter/playbooks/arbitrary-x-authz.md")`（越权矩阵与提权路径）
...
- Mass Assignment 字段清单、HTTP Header 篡改、Cookie 篡改 → 下文对应 REF 章节
...
```
→ 完整 payload 和 Flask session 伪造 → 下文对应 REF 章节
