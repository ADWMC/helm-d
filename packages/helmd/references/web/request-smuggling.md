...
- [ghost-bits-cast-attack](./ghost-bits-cast-attack.md) when the HTTP client library is **Apache HttpClient <= 4.5.9** (HTTPCLIENT-1974/1978) — injecting `瘍瘊` (U+760D U+760A, low bytes `\r\n`) into a header value causes the underlying char-to-byte writer to emit a literal CRLF, splitting the request at the origin without relying on CL/TE disagreement
...
Also load [H2_SMUGGLING_VARIANTS.md](./request-smuggling-h2-smuggling-variants.md) when you need:
...
- **Input enters interpreter/query language/template** (not HTTP framing) -> [Injection Testing Router](./injection-checking.md) (then drill down into XSS, SQLi, SSTI, etc.).
