...
> 来源：wgpsec/AboutSecurity。本文件自包含：redirect_uri 完整 payload、Token 泄露途径、账户接管流程（原 oauth-attack-vectors.md）全部内联。
> 相关主题：[oauth-token-attacks.md](./oauth-token-attacks.md)（state/redirect_uri/implicit/scope + OIDC 核查表）、[jwt-attack.md](./jwt-attack.md)、[saml-sso-assertion-attacks.md](./saml-sso-assertion-attacks.md)。
