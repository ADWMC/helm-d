...
- 认证绕过技巧、参数注入、Mass Assignment、请求走私 → 下文对应 REF 章节
- 403/405 绕过（资源后缀fuzz字典、POST空JSON、Vue Hash路由、前缀发现）→ 下文对应 REF 章节
- 端点语义分析、RESTful CRUD 推断、参数发现、智能 Fuzz、权限边界测试、响应分析 → 下文对应 REF 章节
- 按参数语义分类的 Fuzz payload 模板（ID/查询/文件/金额/命令/Header） → 下文对应 REF 章节
...
→ 完整语义分析方法（参数发现、Content-Type 变体、响应分析、IDOR 批量验证、权限边界测试） → 下文对应 REF 章节
→ 各类型参数的 Fuzz payload 模板 → 下文对应 REF 章节
