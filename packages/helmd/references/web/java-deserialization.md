...
> **SRC / 众测语境** → 用 `read_reference(path: "src-hunter/playbooks/rce/12-deserialization.md")`（gadget 链与版本指纹）
...
- 需要 ysoserial gadget chain 优先级表、URLDNS 检测、payload 构造 → 下文对应 REF 章节
- 需要 JNDI 注入、Fastjson 版本 payload、Shiro/WebLogic/JBoss/Jenkins 专项 → 下文对应 REF 章节
...
- Fastjson：JSON 中含 `@type` 字段 → 下文对应 REF 章节
...
│   ├─ 先 URLDNS 确认漏洞存在 → 下文对应 REF 章节
│   └─ 逐个尝试 CC1-7 → CommonsBeanutils → Spring → Groovy
├─ Fastjson (@type 字段)?
│   └─ 识别版本 → 选择对应 payload → 下文对应 REF 章节
├─ Shiro (rememberMe=deleteMe)?
│   └─ 默认密钥 kPH+bIxk5D2deZiIxcaaaA== → 下文对应 REF 章节
└─ Log4j (Java 应用 + 用户输入)?
    └─ ${jndi:ldap://...} → 下文对应 REF 章节
```
