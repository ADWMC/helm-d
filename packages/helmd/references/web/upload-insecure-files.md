...
Also load [SCENARIOS.md](./upload-insecure-files-scenarios.md) when you need:
...
- [path traversal lfi](./path-traversal-lfi.md) when filename, extraction path, or include path becomes file-system control
- [xss cross site scripting](./xss.md) when uploads are rendered in browser contexts
- [xxe xml external entity](./xxe.md) when SVG, OOXML, or XML imports are accepted
- [cmdi command injection](./command-injection.md) when a processor, converter, or media pipeline executes system tools
- [business logic vulnerabilities](./business-logic-vulnerabilities.md) when quotas, overwrite rules, approvals, or storage paths create logic bugs
- [ghost-bits-cast-attack](./ghost-bits-cast-attack.md) when the server is **Apache Tomcat** and the WAF blocks `.jsp` in `filename*` — Tomcat's `RFC2231Utility` narrows each char to byte, so `1.陪sp` (U+966A low byte = `j`) writes `1.jsp` to disk while the WAF sees no `.jsp` literal
...
| Observation | Pivot |
|---|---|
| SVG or XML accepted | [xxe xml external entity](./xxe.md) |
| filename or metadata reflected | [xss cross site scripting](./xss.md) |
| converter or processor shells out | [cmdi command injection](./command-injection.md) |
| extraction path looks controllable | [path traversal lfi](./path-traversal-lfi.md) |
| overwrite, quota, approval, or tenant bug | [business logic vulnerabilities](./business-logic-vulnerabilities.md) |
