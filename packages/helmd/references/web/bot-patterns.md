...
>
> **详细的分析流程**（攻击面枚举、外泄方式选择、CSP 绕过等）见 `web-methodology.md` §1.5。
> **多步骤攻击编排**（popup 存活、轮询模板、控制器页面）见 `attack-orchestration.md` §3。
> **Bot 代码自动化分析**见 `scripts/bot_analyze.py`。
...
> 自动化提取工具：`python scripts/bot_analyze.py <server.js 路径>`
...
> 各模式的详细时间线、利用条件、攻击编排模板见 `attack-orchestration.md` §3。
...
这对缓存键匹配至关重要 — 详见 `cache-poisoning.md` §7.1 和 `nextjs-analysis.md` §4.1。
...
> 完整的攻击流程（信息收集 → 漏洞发现 → 利用构造 → 验证）见 `web-methodology.md` §1.5。
> 攻击编排模板（控制器页面、postMessage、轮询）见 `attack-orchestration.md`。
