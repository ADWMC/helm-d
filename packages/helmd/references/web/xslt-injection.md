...
> 融合来源：wgpsec/AboutSecurity（方法论主体 + 高级利用参考）。hack-skills 同名篇与本文重复约 95%（处理器指纹/document()/XXE/EXSLT 写/三平台 RCE/决策树均已覆盖），仅保留工具注记并入附录。
> 本文件自包含：XSLT 版本与引擎能力判断、引擎特性矩阵、高级 Payload 分类、验证清单（原 xslt-exploitation.md）全部内联。
> 相关主题：[xxe.md](./xxe.md)（通用 XML 解析器实体加固）、[ssrf.md](./ssrf.md)（`document('http:…')` 触发服务端请求）。
...
- XSLT 高级利用 payload 与引擎特性（XSLT 2.0+/盲利用/WAF 绕过） → 下文对应 REF 章节
...
> 融合自 yaklang/hack-skills `xslt-injection`，其余内容与上文重复，已并入对应章节。

- **PayloadsAllTheThings 注记**：XSLT 没有类似 SQLi/XSS 的成熟通用工具链 —— 利用高度依赖处理器与配置，靠代理重放 + 自写脚本。尽量在与目标相同引擎/版本的本地环境先复现。
- **工具**：Burp/ZAP 重放样式表观察报错；OOB 用 Collaborator / 自建回调收 `document('http://…')`；无万能扫描器能替代版本级行为验证。
- 没有打出 RCE ≠ 没有漏洞：`document()` 文件读、SSRF、EXSLT 写文件在加固配置下依然常见。
