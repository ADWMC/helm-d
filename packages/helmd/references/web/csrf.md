...
- [cors cross origin misconfiguration](./cors-cross-origin-misconfiguration.md) when JSON endpoints become readable cross-origin
- [oauth oidc misconfiguration](./oauth-token-attacks.md) when login, account linking, or callback binding relies on OAuth state
...
# 补充：CSRF 方法论速查（中文，融合自 wgpsec/AboutSecurity）
...
高价值目标操作：改密码/邮箱（→ 账户接管）、转账/支付、添加管理员、绑定/解绑第三方账户、修改权限/角色。

## Referer/Origin 绕过（正文未覆盖）
