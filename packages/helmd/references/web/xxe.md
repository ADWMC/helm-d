...
- [upload insecure files](./upload-insecure-files.md) when XXE is reachable through SVG, OOXML, import, or preview pipelines

### Extended Scenarios

Also load [SCENARIOS.md](./xxe.md) when you need:
...
# 补充：XXE 方法论速查（中文，融合自 wgpsec/AboutSecurity `xxe-injection`，与正文去重后仅留独有项）

## JSON → XML 转换攻击

后端同时支持 JSON 和 XML、按 Content-Type 自动选择解析器时，改发 XML：
...
即使文档只说 JSON，也要试 XML Content-Type —— 隐式支持很常见。

## SAML 中的 XXE

SAML 断言是 XML：把 DOCTYPE+ENTITY 注入 SAML Response，在 `<saml:Issuer>` 等元素引用 `&xxe;`，注意保持 namespace 完整。
...
**UTF-16**：`iconv -f UTF-8 -t UTF-16 payload.xml > payload_utf16.xml`
...
| MSXML 3.0 / 6.0 | 3.0 启用，6.0 禁用 | 6.0 需手动 `ProhibitDTD=true` |
...
读取远程 ZIP 内文件时写入 `/tmp/`，可配合路径遍历进一步利用。
