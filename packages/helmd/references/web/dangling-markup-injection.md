...
- [xss-cross-site-scripting](./xss.md) when full XSS is possible (no need for dangling markup)
- [csp-bypass-advanced](./csp-bypass-advanced.md) when CSP blocks JS execution — dangling markup bypasses script restrictions
- [csrf-cross-site-request-forgery](./csrf.md) when dangling markup steals CSRF tokens for subsequent CSRF attacks
- [crlf-injection](./crlf-injection.md) when CRLF enables HTML injection in HTTP response
- [web-cache-deception](./web-cache-deception.md) when dangling markup + cache poisoning amplifies the attack
