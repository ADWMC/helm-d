...
| 攻击面 | 检查方法 | 典型漏洞 |
|--------|---------|---------|
| max_input_vars | 发送 >1000 个参数，观察响应是否缺少安全头 | 安全头绕过（详见 `csp-bypass.md`） |
...
> 完整的攻击编排模板（含 iframe 逃逸、localStorage 轮询、自动外泄），见 `attack-orchestration.md` §3.3。
...
> 详细的攻击编排模式见 `attack-orchestration.md`
...
### 2.3 SPA 黑盒评估（React/Vue/Angular）

SPA 应用的所有 API 路由和认证逻辑都打包在前端 JS bundle 中，可通过静态分析提取。

```
1. 下载主 JS bundle
   curl -s http://target/ > /tmp/page.html
   grep -oP 'src="([^"]+\.js)"' /tmp/page.html     # 找 chunk 路径
   grep -oP 'href="([^"]+\.css)"' /tmp/page.html    # 找 CSS（确认框架）
   curl -s http://target/assets/index-XXXXX.js > /tmp/app.js

2. 提取 API 端点
   grep -oP '"/api/[^"]+"|"/[a-z]+/[a-z]+"' /tmp/app.js | sort -u
   # Python 精确提取（处理混淆后的字符串拼接）
   python3 -c "
   import re
   js = open('/tmp/app.js').read()
   routes = re.findall(r'\"(/[a-zA-Z0-9/_\-\${}]+)\"', js)
   api_routes = [r for r in routes if '/api/' in r or r.count('/') >= 2]
   for r in sorted(set(api_routes)):
       print(r)
   "

3. 识别认证机制
   - 搜索 localStorage/sessionStorage → token 存储位置
   - 搜索 Authorization/Bearer → token 传递方式
   - 搜索 axios/fetch wrapper → 请求拦截器位置
   - 搜索 401/403 → 未授权处理逻辑
   - 搜索 hash 路由 → 前端路由守卫（#/login, #/dashboard）

4. 逐端点测试
   无认证 → 信息泄露（优先级最高）
   无效 token → 错误处理
   不同 HTTP 方法 → 方法限制（如 GET 端点是否允许 POST）
   CORS → Access-Control-Allow-Origin / Credentials
   速率限制 → 连续 10+ 请求观察 429/锁定

5. JWT 深度分析（如有）
   - 解码 header/payload（base64url decode 前两段）
   - 检查 exp/nbf/iss 字段
   - 暴力弱密钥（常见 secret 列表）
   - 算法混淆测试（none / HS256↔RS256）
```

**CORS Origin Reflection 测试（关键）**：

当 `Access-Control-Allow-Origin` 反射请求中的 `Origin` 头，且 `Access-Control-Allow-Credentials: true` 时，任何网站可对已登录用户发起跨域认证请求并读取响应。这是 Critical 级漏洞。

```bash
# 测试方法：发送任意 Origin，检查是否被反射
curl -sk -H 'Origin: https://evil.com' -I https://target/api/v1/endpoint | grep Access-Control
# 危险: Access-Control-Allow-Origin: https://evil.com + Credentials: true
# 安全: Access-Control-Allow-Origin: * (不含 Credentials) 或固定白名单
```

与 `Access-Control-Allow-Origin: *` 的区别：通配符不配 Credentials 时浏览器不发送 Cookie，实际风险低于预期。但 **Origin 反射 + Credentials** = 完整数据窃取。

**SPA 特有陷阱**：
- Vite 构建的 SPA chunk 文件名含哈希（如 `index-DzsQKNg-.js`），需先从 HTML 提取路径
- API wrapper 可能统一处理认证（如 `Ne(method, url, body)` 函数自动加 Bearer header）
- 前端路由（hash 或 history 模式）决定页面结构，不影响 API 路由
- localStorage token 可被 XSS 直接读取，比 httpOnly cookie 更危险
- CORS `Access-Control-Allow-Origin: *` **不配** `Allow-Credentials: true` 时，浏览器不会跨域发送 Cookie/Auth 头，实际风险低于预期。但仍可跨域读取响应数据（如 /api/stats），且若存在同源 XSS 则 token 可被窃取

**Nuxt.js + PHP 混合架构陷阱**：
- Nuxt SSR 应用（如 CRMEB）会 fallback 所有未匹配路径到 homepage HTML（HTTP 200）
- 判断真实 PHP API：发送 `Accept: application/json` header，检查 `Content-Type: application/json`
- CRMEB 的 `/api/*` 路径是真实 PHP 后端（返回 JSON）；`/user/api/*`、`/admin/*` 被 Nuxt 接管（返回 HTML）
- 详细 CRMEB 侦察流程见 `crmeb-recon.md`

**Node.js/Express API 常见未认证端点**：
- `/api/stats` / `/api/health` / `/api/version` — 管理面板常遗漏认证
- `/api/logs` — 日志查询常被标记为"只读"而跳过认证检查，泄露用户名、IP、操作记录
- `/api/logout` — 登出通常不需要认证（功能上可接受，但不一致）
- `/api/config` — 配置端点有时暴露环境变量或数据库连接信息

**JWT 常见问题**：
- 无 `exp` 字段 — token 永不过期，泄露后无法自动失效
- HS256 弱密钥 — 暴力常见 secret 列表（secret/password/123456/admin/shvmp/jwt_secret）
- 存储在 localStorage — XSS 可直接读取，应改用 httpOnly cookie
