...
- 编码绕过 Payload（双重 URL/Unicode/HTML/混合编码） → 下文对应 REF 章节
- HTTP 协议层绕过（分块传输/Content-Type/方法切换/HTTP2/走私） → 下文对应 REF 章节
- 参数层绕过（HPP/数组语法/Multipart） → parameter-bypass（该参考文件未入库）
- Payload 变形（通用编码/拆分/格式切换） → 下文对应 REF 章节
