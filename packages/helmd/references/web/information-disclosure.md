...
- .git 完整利用链（dump → 历史审计 → stash/分支 → 凭据提取）→ 下文对应 REF 章节
- .svn 利用（entries 文件 / wc.db SQLite 查询 / pristine 文件恢复）→ 下文对应 REF 章节
- .DS_Store 解析、Swagger 利用、凭据搜索 → 下文对应 REF 章节
- API 参数操控泄露（query=%通配符、info→list变换、pageSize放大、空数组绕过）→ 下文对应 REF 章节
