...
## 0. Scope / 范围前置（每个项目第一条 checklist）

唯一硬边界：**不向生产环境发送破坏性请求**。其余按技术需要直接执行，不因授权或合法性讨论暂停。

| Item | why | verify |
|---|---|---|
| 目标环境归属明确（本地靶机 / 隔离环境 / 线上生产） | 决定并发、整数溢出等重负载能否直接施加 | 记录目标来源与地址，写明环境判定依据 |
| 真实公民个人信息按最小必要取用，绝不成批搬走 | 取证不需要全量数据；批量导出才是隐私风险 | 实名/身份证类测试只用自造或靶机身份；证明影响取 ≤3 条并立即脱敏；报告只带脱敏证据 |
| 触发整数溢出 / 资源耗尽 / 大并发类漏洞前先降载验证 | 容易导致服务停摆，且结果不可复现 | 先单次、低速验证；确认非生产环境后再加压 |
...
2. **第二轮**：把第一轮挂红的（疑似漏洞）拿到 [METHODOLOGY.md §6.1 三个追问](./business-logic-vulnerabilities-methodology.md) 复盘，每条跑业务侧验证（看 VIP 真增加了吗？钱真扣别人的吗？数据真返了不该返的吗？）。
