...
| 当...时 | 使用本节 |
|---------|---------|
| 用户输入直接拼接进 SQL/命令/模板 | §1 注入类 |
| 页面中存在用户可控的 HTML/JS 输出 | §2 XSS |
| 应用请求用户可控的 URL/路径 | §3 SSRF |
| 存在跨站请求伪造风险 | §4 CSRF |
| Cookie/Token 可被窃取或滥用 | §5 会话管理 |
...
> SSO 审计的完整攻击编排流程（含 iframe sandbox 逃逸 + blob URL 组合利用），见 `attack-orchestration.md` §4。
