...
- Pickle payload 模板库（回显/盲/文件上传/发送方式）→ 下文对应 REF 章节
- RCE 成功但无回显？盲利用外带策略 → 下文对应 REF 章节
- PHP 反序列化详解（POP Chain/Type Juggling/phpggc） → 下文对应 REF 章节
- .NET 反序列化利用（ViewState/BinaryFormatter/ysoserial.net） → 下文对应 REF 章节
...
→ 完整模板 → 读 下文「REF: pickle-payload-templates」
...
→ 完整 POP Chain/Type Juggling/phpggc 用法 → 读 下文「REF: php-deserialization」
