...
- [waf-bypass-techniques](./waf-bypass-techniques.md) — when a Java
  backend is suspected and WAF rules block the literal payload, this is the
  first technique to try beyond classic encoding.
- [deserialization-insecure](./deserialization-insecure.md) — for
  Apache Commons BCEL ClassLoader and Fastjson `\u`/`\x` escape variants.
- [path-traversal-lfi](./path-traversal-lfi.md) — Spring, Jetty,
  Undertow, Vert.x URL decoding and `%2>` hex folding.
- [upload-insecure-files](./upload-insecure-files.md) — Tomcat
  `RFC2231Utility` `filename*` Webshell upload.
- [request-smuggling](./request-smuggling.md) — Apache HttpClient
  `<= 4.5.9` (HTTPCLIENT-1974/1978) header CRLF.
- [crlf-injection](./crlf-injection.md) — Angus Mail / Jakarta Mail
  SMTP injection and JDK HttpServer response splitting.
- [sqli-sql-injection](./sql-injection.md) — Jackson `charToHex`
  table-lookup truncation hides SQL keywords inside Unicode escapes.

### Advanced Reference

Load [PAYLOAD_COOKBOOK.md](./ghost-bits-cast-attack-payload-cookbook.md) when you need:
...
Pair with [sqli-sql-injection](./sql-injection.md) for the
...
[crlf-injection](./crlf-injection.md) for non-mail CRLF reuse.
...
Cross-reference [request-smuggling](./request-smuggling.md) for
...
See [PAYLOAD_COOKBOOK.md](./ghost-bits-cast-attack-payload-cookbook.md) for affected-version
