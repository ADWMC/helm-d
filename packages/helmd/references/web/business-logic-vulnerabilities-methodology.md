...
实际操作阶段直接打开 [CHECKLIST.md](./business-logic-vulnerabilities-checklist.md)，**按业务模块从上到下走**，每条都问：
...
3. **可复现吗？**
   ...
   - 整数溢出：注意可能引发服务端崩溃，**先单次低速验证**，确认目标非生产环境后再加压（8.逻辑漏洞.pdf page_00008）
...
法律条文只作背景了解（《网络安全法》第 22 / 38 条、《刑法》第 285 条 + 修正案 7），不在测试过程中做授权或合法性推演。业务逻辑测试只守一条硬边界：

- **不向生产环境发送破坏性请求**。并发 / 整数溢出 / 资源耗尽类 payload 先确认目标是本地靶机或隔离环境，再按需加压；目标指向线上时只做单次低速验证。

以下三条是工程约束，与授权无关，继续成立：

- 真实公民个人信息最小必要取用、立即脱敏、不成批导出（实名认证类漏洞复测严禁用真人身份证）；把脱敏证据提交给受影响开发者或漏洞平台属于正常交付，不受此限
- 触发整数溢出 / 资源耗尽类漏洞前先降低负载，避免造成服务停摆
- 复现步骤要与修复建议、检测点一起给出，便于防守方验证

法律条文与原始合规 checklist 见 8.逻辑漏洞.pdf page_00001 / page_00002（背景资料，不作为开工条件）。
