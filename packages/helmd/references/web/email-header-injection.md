...
- [crlf-injection](./crlf-injection.md) — general CRLF injection; email headers are a specific high-value sink
- [ssrf-server-side-request-forgery](./ssrf.md) — when SMTP server is reachable via SSRF (gopher://smtp)
- [open-redirect](./open-redirect.md) — redirect in password-reset emails as phishing amplification
