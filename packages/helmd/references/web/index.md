...
**来源说明（续）**：第三批融合 [MyuriKanao/src-hunter-skill](https://github.com/MyuriKanao/src-hunter-skill)（SRC/众测语料）
**不并入本目录**，而是原样保留在子目录 [`src-hunter/`](src-hunter/index.md) 内 —— 因为它是按需启用的独立能力层，
与上面两批「按主题合并进 web 域」的处理方式不同。详见文末「src-hunter 子目录」一节。
...
[`src-hunter/index.md`](src-hunter/index.md) — 融合自 [MyuriKanao/src-hunter-skill](https://github.com/MyuriKanao/src-hunter-skill)（MIT）。
...
| `src-hunter/compliance.md` + `templates/` | 合规红线 + 三段式报告模板（CVSS 4.0） |
