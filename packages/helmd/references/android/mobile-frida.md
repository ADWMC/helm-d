...
```bash
# 一键安装（自动下载 + 随机名 + 随机目录）
python scripts/manage_frida.py --action install -s <serial>

# 启动（自动分配端口 + 端口转发，输出 JSON）
python scripts/manage_frida.py --action start -s <serial>

# 检查状态
python scripts/manage_frida.py --action status -s <serial>

# 停止
python scripts/manage_frida.py --action stop -s <serial>
```
...
# 3. 生成随机名
RANDOM_NAME=$(python -c "import secrets; print(secrets.token_hex(4))")
...
> device.json 是任务级设备快照，记录当前任务绑定的设备。由 Agent 在首次连接设备时创建，放在 `task-dir/device.json`。
...
2. task-dir/device.json 不存在 → 进入选择流程
...
> - **Python SDK**: 必须用 `frida.Compiler` 编译 TypeScript（`import Java from "frida-java-bridge"`），详见 `frida-17x-bridge.md`
...
> - **Python SDK**: 必须用 `frida.Compiler` 编译 TypeScript（`import ObjC from "frida-objc-bridge"`），详见 `frida-17x-bridge.md`
