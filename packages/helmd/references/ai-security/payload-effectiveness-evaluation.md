...
> 不依赖主 prompt 上下文即可理解。
>
> 前置知识：`llm-attack-methodology.md` §2.3（结果分析维度）、`carrier-construction-guide.md`（载体质量标准）

| 当...时 | 使用本节 |
|---------|---------|
| 注入成功但怀疑输出不真实 | §2 真实性评估维度 |
| 需要对比注入前后输出差异 | §3 对比分析方法 |
| 需要量化攻击效果 | §4 评分卡方法 |
| 注入失败需要诊断 | §5 常见失败模式 |
...
**解决方案**：升级载体质量。详见 `carrier-construction-guide.md`。
...
**解决方案**：降低暗示强度。参考 `llm-attack-methodology.md` §3.4 的操纵检测阈值。
