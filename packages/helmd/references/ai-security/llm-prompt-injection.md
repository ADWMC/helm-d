...
- [ai-ml-security](./ai-ml-security.md) for broader ML security (adversarial examples, model poisoning, model extraction, data privacy attacks)
- [xss-cross-site-scripting](../web/xss.md) for parallels between XSS (injecting into HTML context) and prompt injection (injecting into LLM context)
- [ssrf-server-side-request-forgery](../web/ssrf.md) when prompt injection chains into SSRF via tool calls

### Advanced Reference

Also load [JAILBREAK_PATTERNS.md](./llm-prompt-injection-jailbreak-patterns.md) when you need:
...
Effectiveness varies by model and version. See [JAILBREAK_PATTERNS.md](./llm-prompt-injection-jailbreak-patterns.md) for comprehensive catalog.
