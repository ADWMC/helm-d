...
- **360 jiagu 加固 app：`Java` is not defined**。jiagu hook 了 JNI 环境，Frida attach 后整个 `Java` 对象不存在（不是 `Java.perform` 失败，而是 `Java` 本身 undefined）。解决：不用 Java Bridge，用纯 Native API（`Process.enumerateModules()` + `addr.readByteArray()`）dump 解密后的 DEX。见 android 域 `android-unpacking.md`
