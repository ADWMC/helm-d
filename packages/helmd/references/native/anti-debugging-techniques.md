...
- [code-obfuscation-deobfuscation](./code-obfuscation-deobfuscation.md) when the binary also uses control flow flattening, VM protection, or string encryption
- [vm-and-bytecode-reverse](./vm-and-bytecode-reverse.md) when the anti-debug sits inside a custom VM dispatcher
- [symbolic-execution-tools](./symbolic-execution-tools.md) when you want to symbolically skip anti-debug checks entirely

### Advanced Reference

Also load [ANTI_DEBUG_MATRIX.md](./anti-debugging-techniques-anti-debug-matrix.md) when you need:
