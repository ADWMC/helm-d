...
- [container-escape-techniques](./container-escape-techniques.md) when the target is a container and you need to escape to host
- [linux-security-bypass](./linux-security-bypass.md) when facing restricted shells, AppArmor, SELinux, or seccomp
- [linux-lateral-movement](./linux-lateral-movement.md) after obtaining root for pivoting to adjacent hosts
- [kubernetes-pentesting](./kubernetes-pentesting.md) when the host is a Kubernetes node

### Advanced Reference

Also load [SUID_CAPABILITIES_TRICKS.md](./linux-privilege-escalation-suid-capabilities-tricks.md) when you need:
...
Also load [KERNEL_EXPLOITS_CHECKLIST.md](./linux-privilege-escalation-kernel-exploits-checklist.md) when you need:
