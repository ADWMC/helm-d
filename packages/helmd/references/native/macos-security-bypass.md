...
- [macos-process-injection](./macos-process-injection.md) when you need dylib injection, XPC exploitation, or Electron abuse after achieving initial access
- [linux-privilege-escalation](./linux-privilege-escalation.md) for Unix-layer privesc techniques that also apply to macOS (SUID, cron, writable paths)
- [linux-security-bypass](./linux-security-bypass.md) for shared Unix security bypass concepts

### Advanced Reference

Also load [TCC_BYPASS_MATRIX.md](./macos-security-bypass-tcc-bypass-matrix.md) when you need:
