...
- [stack-overflow-and-rop](./stack-overflow-and-rop.md) — ROP chains to bypass NX, ret2libc for ASLR bypass
- [format-string-exploitation](./format-string-exploitation.md) — primary method for leaking canary, PIE, libc addresses
- [heap-exploitation](./heap-exploitation.md) — heap attacks for RELRO bypass (when GOT is read-only)
- [arbitrary-write-to-rce](./arbitrary-write-to-rce.md) — what to overwrite when GOT is protected by RELRO

### Advanced Reference

Load [PROTECTION_BYPASS_MATRIX.md](./binary-protection-bypass-protection-bypass-matrix.md) for comprehensive protection × bypass × primitive matrix.
...
See [arbitrary-write-to-rce](./arbitrary-write-to-rce.md) for comprehensive target list.
