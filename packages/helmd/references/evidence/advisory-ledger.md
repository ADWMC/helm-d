...
第三种是"值得说几次、不值得说到永远"的纪律：`adaptive` 只在该 key 的**最近 10 条**记账里样本 <5 或采纳率 <70%
时渲染提醒，达标后提醒退场、计量继续（`shouldTeach()`）。**教与测是同一次 submit**，退场的只是那段正文。
用滑窗而不是累计率：累计率在足够长的好历史之后永远掉不下来（后期回归就再也不会教），贴近阈值时还会抖。
...
- 退场（`adaptive`）：同 key **最近 10 条**里 ≥5 样本且采纳率 ≥70% → 不再渲染提醒，账本继续记；
  窗口内一旦退化（<70%）提醒自动回来。
- 触发（`stance:report-prefix`）：需要**两个信号同时成立**——问到我们的工作（progress / 汇报 / 进展 / 修完…）
  且是问句形态（`?` / 如何 / 了吗 / any / what…）。裸 `状态`/`status` 刻意不算名词："查看服务状态"是任务，
  误武装会把普通回复记成 ignored。
