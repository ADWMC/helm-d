...
OpenCode serve 需要在本地运行（默认 `127.0.0.1:4096`）。在 OpenCode 中使用此工具时，服务已经在运行。
...
```bash
python ai_dialogue_main.py create -t <模型ID> --provider opencode-go --title "会话标题"
```
...
```bash
python ai_dialogue_main.py send -s <session_id> -p "消息内容"
```
...
```bash
python ai_dialogue_main.py chat -t <模型ID> --provider opencode-go -p "消息"
```
...
```bash
python ai_dialogue_main.py list
```

### messages — 查看会话历史

```bash
python ai_dialogue_main.py messages -s <session_id>
```

### summarize — 压缩上下文

```bash
python ai_dialogue_main.py summarize -s <session_id>
```
...
```bash
python ai_dialogue_main.py delete -s <session_id>
```
