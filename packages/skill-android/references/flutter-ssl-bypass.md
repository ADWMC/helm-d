...
# 详见 arm64-reverse-methodology.md
import idautils
import idc
import ida_bytes
...
| `process crashed` | 应用检测到 Frida | 反检测措施见 `mobile-frida.md` |
...
// 以下偏移量必须从目标 Flutter 应用的 IDA 数据库中确认
// 搜索 "TrustBuiltinRoots" 和 "peer_extract" 等字符串定位函数地址
var TRUST_BUILTIN_ROOTS_OFFSET = 0x0;  // 替换为 IDA 中的实际偏移（如 0x3A2B10）
var PEER_EXTRACTOR_OFFSET = 0x0;        // 替换为 IDA 中的实际偏移（如 0x3C4D20）
