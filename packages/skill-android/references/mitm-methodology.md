...
| 当...时 | 使用本节 |
|---------|---------|
| 需要选择 MITM 方案 | §1 四种 MITM 方案对比 |
| 配置自签 CA | §2 CA 证书安装 |
| 遇到 SSL Pinning 阻碍 | §3 SSL Pinning 绕过 |
| MITM 失败需要排查 | §4 常见陷阱 |
...
使用 `scripts/mitm_proxy.py`（已沉淀的通用脚本）:

```bash
# --workdir 是必须参数，用于存放 CA 密钥和证书
python3 scripts/mitm_proxy.py \
  --workdir /tmp/mitm \
  --listen-port 44300 \
...
需要三部分 Hook：
1. **SSL bypass** — 详见 `flutter-ssl-bypass.md` §6 完整模板（TrustBuiltinRoots + native peer 捕获）
...
// 完整 connect+read 拦截模板见 tls-traffic-interception.md
// 响应篡改（popen+curl）见 frida-native-shell-tricks.md §2.2
...
| Frida 崩溃 | Java bridge 不稳定（Flutter 应用） | 用 native popen/fgets 替代 Java bridge，详见 `frida-native-shell-tricks.md` |
...
- Flutter SSL bypass 详见 `flutter-ssl-bypass.md`
- TLS 流量拦截（connect 追踪 fd + read 检测 AppData）详见 `tls-traffic-interception.md`
- Frida native shell 技巧详见 `frida-native-shell-tricks.md`
- Frida 反检测和设备操作详见 `mobile-frida.md`
