...
| 当...时 | 使用本节 |
|---------|---------|
| 识别到 Next.js 框架 | §1 识别 Next.js App Router |
| 分析 RSC/flight data | §2 RSC 安全分析 |
| 审计 middleware | §3 Middleware 审计 |
| 需要读 node_modules 源码 | §4 框架源码审计 |
| 分析 SSRF/注入漏洞 | §5 常见漏洞模式 |
...
> 缓存投毒的完整方法论（缓存键探测、AE 差异利用、缓存中缓存、Bot AE 探测）见 `cache-poisoning.md`。
> 自动化探测脚本见 `scripts/cache_poison.py`。
